...
namespace CcNet.Utils.Extensions
{
    ...
    public static class StringExtension
    {
        ...
        /// <returns></returns>
        public static string UpperCase(this string str, bool trimSapce = true)
            => str.GetValue(trimSapce).ToUpper();
        ...
        /// <param name="trim">是否移除前后空格</param>
        /// <returns></returns>
        public static string LowerCase(this string str, bool trimSapce = true)
            => str.GetValue(trimSapce).ToLower();
        ...
        /// </summary>
        /// <param name="str"></param>
        /// <param name="defaultColor">转换失败时返回的默认值</param>
        /// <returns></returns>
        public static Color ToColor(this string str, Color defaultColor)
        {
            try
            {
                ...
                var offset = str.StartsWith("#") ? 1 : 0;

                //转换颜色
                return Color.FromArgb(
                    int.Parse(str.Substring(offset, 2), NumberStyles.AllowHexSpecifier),
                    int.Parse(str.Substring(offset + 2, 2), NumberStyles.AllowHexSpecifier),
                    int.Parse(str.Substring(offset + 4, 2), NumberStyles.AllowHexSpecifier));
            }
            catch (Exception ex)
            {
                return defaultColor;
        ...
        /// <returns>宽度</returns>
        public static int GetPixesSize(this string str, Font font, out int height)
        {
            var size = TextRenderer.MeasureText(str, font, new Size(0, 0), TextFormatFlags.NoPadding);
            height = size.Height;
            return size.Width;
        }
